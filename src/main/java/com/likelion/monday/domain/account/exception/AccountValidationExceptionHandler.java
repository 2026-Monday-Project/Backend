...
/**
 * 계정/로그인 요청의 입력값 검증 실패를 프론트가 그대로 보여줄 수 있는 메시지로 변환한다.
 * GlobalExceptionHandler는 Exception까지 잡아 500으로 내리므로, 그보다 먼저 동작하도록 우선순위를 올린다.
 * 추후 공통 예외 처리에 동일 핸들러가 추가되면 이 클래스는 삭제한다.
 */
@Slf4j
@Order(Ordered.HIGHEST_PRECEDENCE)
@RestControllerAdvice(basePackages = "com.likelion.monday.domain.account")
public class AccountValidationExceptionHandler {
