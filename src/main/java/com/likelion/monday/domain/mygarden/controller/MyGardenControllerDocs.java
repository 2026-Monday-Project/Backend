...
public interface MyGardenControllerDocs {
    ...
    @Operation(summary = "내 사연 미리보기 조회", description = "마이페이지 요약용으로 최신 사연 2건만 조회한다.")
    ApiResponse<List<MyStorySummaryResDto>> getMyStoriesPreview(Long accountId);

    @Operation(
            summary = "내 사연 전체/필터 조회",
            description = "status를 생략하면 전체(ALL), 지정하면 검토중/공개/비공개로 필터링해 최신순으로 조회한다."
    )
    ApiResponse<List<MyStorySummaryResDto>> getMyStories(Long accountId, StoryStatus status);
}